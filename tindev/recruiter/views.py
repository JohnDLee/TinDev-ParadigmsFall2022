...
class PostView(LoginRequiredMixin, UserPassesTestMixin, DetailView):
    ...
    def test_func(self):
        if recruiter_check(self.request.user):
            try:
                self.recruiter = RecruiterProfile.objects.get(user = self.request.user)
                return True
            except RecruiterProfile.DoesNotExist:
                # no recruiter exists
                HttpResponseRedirect("/recruiter/profile_creation/")
...
def post_update_view(request, pk = None):
    ...
    if request.method == 'POST':
        form = JobPostCreationForm(request.POST, initial=init, prefix="update_post")
        
        if form.is_valid():

            # save and redirect to recruiter page
            form.update(recruiter=recruiter,prev_candidates=init['interested_candidates'], pk = pk)
